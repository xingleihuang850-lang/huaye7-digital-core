...
> **M7-v3 当前状态**：见 [`notes/24_阶段二_M7v3_连通性迭代设计.md`](notes/24_阶段二_M7v3_连通性迭代设计.md) 与 [`notes/25_阶段二_M7v3_200ep评估与B1决策.md`](notes/25_阶段二_M7v3_200ep评估与B1决策.md)。`ddpm_ct28_200ep` 已完成采样/评估：200ep T=0 为 φ=4.960%、S₂ rmse=0.00372、Euler=120.88、maxCC=0.0467；T* 标定到 φ=6.4 后 S₂ rmse=0.01010、Euler=119.58。结论：单纯二值 DDPM 加 epoch 不闭合；下一步转 **B1 生成灰度介质 sus 本身**，B2 `[sus,pore]` 双通道作为备选。注意：真实 sus 输入→pore 输出是分割，不是生成。
