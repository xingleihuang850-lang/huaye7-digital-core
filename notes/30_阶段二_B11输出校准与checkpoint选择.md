...
### 15.24 qmatch-conditioned minimal 3D smoke 设计纳入

已按研究判断调整 Stage 3/A2 设计重点：2D gate 已基本打扫干净，后续最大不确定性不是继续包装 2D 成绩，而是 qmatch 条件下升维后的 3D connectivity 与 physical-response proxy 是否成立。因此新增 fresh-gated minimal 3D smoke 设计与 checklist，并同步更新 A2 design card、execution checklist、metric interface schema 与 risk register。
...
Smoke scope 仍未授权执行：`SMOKE_DESIGN_ONLY_EXECUTION_NOT_AUTHORIZED`。若未来 gate 允许，smoke 也只能是 diagnostic smoke not evidence，候选体积偏向 `<=64^3`，最多 `<=128^3` 且必须显式 gate 批准；candidate_count <= 3；route 必须保留 `nnUNet ep015_qmatch` / `hy7-gray-calibration-qmatch-v1`，formal `ep015_all` 仅为 planning anchor。

Smoke 必须输出 3D connectivity metrics 与 physical-response proxy，并显式记录 non-percolating axes、connected porosity、LCC、S2 boundary disagreements、Euler/Minkowski status、formal-vs-qmatch disagreement、failed chunk `ep015_chunk000_063` relation 等 negative evidence。

Risk register 新增 A2-R36–A2-R42：qmatch-conditioned 3D connectivity failure、physical-response invalidity、smoke scope creep、qmatch-conditioned permeability overclaim、smoke negative evidence suppression、formal-anchor/qmatch disagreement、non-reproducible smoke volume。

### 15.25 A2/smoke design completion gate 与条件接口纳入

已运行 strict `digital-rock-gate` completion review。Gate question 不是“能否 A2 execution”，而是：

```text
现有 A2/smoke design package 是否完整到可以请求一个 fresh-gated minimal 3D smoke？
```
...
```text
experiments/花页7_PlanB_记录/phase2/b2_min_calibrated/stage3_condition_interface_design_20260706.md
...
本 gate 只授权“提交未来 minimal 3D smoke gate request”，不授权执行 smoke，不授权 A2-small/A2-medium，不授权数字井筒 claim。

MoA 明确要求未来 smoke gate request payload 必须携带以下约束：

- per-channel condition declaration 必须具体填写：depth/mineral/log/electrical-imaging = used / held / de-scoped / not available，且每项有一句 rationale；blank channel => fail closed；
- smoke metric set 必须逐项列出，并带 pass/fail/negative-evidence slots：3D porosity、connected porosity、x/y/z percolation、LCC fractions、boundary-labelled S2 x/y/z、Euler/Minkowski status；
- negative-evidence protocol 必须说明如何记录和暴露，而不只是说“需要 negative evidence”；
- request 本身必须 stamp：`not_a_generative_digital_well=true`，且 failed chunk `ep015_chunk000_063` 是 visible negative evidence。

条件接口纳入结论：当前条件接口 / representativeness / conditional-response layer 对这个阶段是 adequate：足以约束 minimal local 3D smoke，防止把 smoke 误称为 digital well；但真正 digital-well claim 仍必须等未来 `condition_manifest.json`、`representativeness_audit.json`、`conditional_response_plan.json`、conditional response gate / metrics 完成后才可能讨论。
...
### 15.30 inter-slice audit gate request 与 blocking fixes

对 §15.29 的 metadata-only inter-slice consistency audit plan 发起 strict `digital-rock-gate` gate request：

```text
experiments/花页7_PlanB_记录/phase2/b2_min_calibrated/stage3_inter_slice_consistency_audit_gate_request_20260707.md
experiments/花页7_PlanB_记录/phase2/b2_min_calibrated/stage3_inter_slice_consistency_audit_gate_moa_output_20260707.md
experiments/花页7_PlanB_记录/phase2/b2_min_calibrated/stage3_inter_slice_consistency_audit_gate_record_20260707.md
```
...
Blocking issue：原 plan/checklist 虽然整体是 diagnostic metadata-only，但包含 downstream second-smoke implication language（例如 `Candidate for second-smoke request only after new gate` 与 checklist `second_smoke_request_candidate_only_if`），触发 request 自身 fail-closed 条件。MoA 允许的下一步仅为 planning artifact correction，不允许实现/执行 audit launcher/package。
...
修订要点：移除 second-smoke unlock language；新增 no_second_smoke_implication / no_a2_small_implication；预注册诊断阈值（Jaccard x/yz <0.25、S2 x/min(y,z) <0.25、pairwise persistence <0.10、per-slice robust z >3.5、near-zero pore slice run >=3）；axis 0 → 384:448 slice-stacking mapping 必须 first-check fail-closed；component/run-length 仅为 adjacent-slice 2D-pairwise proxy，不是 3D percolation；future implementation 必须 hash re-verify、禁止 torch/tensorflow/nnunet import、不得中间 array persistence。

### 15.31 corrected inter-slice audit gate 与 run01 metadata package

修订后重新发起 gate：

```text
experiments/花页7_PlanB_记录/phase2/b2_min_calibrated/stage3_inter_slice_consistency_audit_gate_rerequest_20260707.md
experiments/花页7_PlanB_记录/phase2/b2_min_calibrated/stage3_inter_slice_consistency_audit_gate_rerequest_moa_output_20260707.md
```
...
仍不授权 second smoke / A2-small / A2-medium / full 2D→3D / training / checkpoint / scientific acceptance / qmatch formal acceptance / validated permeability / generative digital-well claim。

按 verdict 实现并执行 metadata-only 9-file audit package：
...
experiments/花页7_PlanB_记录/phase2/b2_min_calibrated/stage3_inter_slice_audit_run01_record_20260707.md
```

Source hash re-verify：

```text
remote=hy7-linux:/home/user/HXL/HY7_planb/phase2/b1_gray_sus_b11_rescue20_nnunet_review_20260706/ep015_qmatch_pore_nnunet2d.npy
sha256=24655b96b3f50ad14f8c60548afa9e0169babb76c24b605efa870e741502b7b2
```
...
### 15.35 OpenCode 代码线交接与全局代码总览修正

用户修正：OpenCode 的代码编写/解读/远程 Linux 状态分析不是 B2-min calibrated 阶段证据，不能放在 `experiments/花页7_PlanB_记录/phase2/b2_min_calibrated/` 下。该目录下的 `opencode_analysis_20260707/` 已删除；此前分析仅作为迁移材料保存在本地总览工作区。

本地 OpenCode 总览根目录：

```text
/Users/hxl/Documents/Hermes工作区/01_Claude接管剩余工作/OpenCode数字井筒代码总览/
```

迁移保留的首轮分析：

```text
/Users/hxl/Documents/Hermes工作区/01_Claude接管剩余工作/OpenCode数字井筒代码总览/20260707_initial_audit/
```

后续 OpenCode 对“接管 Claude 剩余工作”里数字井筒研究主线涉及的所有代码、远程 Linux `/home/user/HXL/HY7_planb` 实验过程、实验结果、数据/权重/日志/manifest、代码来源和理论依据的统一分析，都写入这个本地总览根目录；不要再写入单个实验阶段目录。Hermes 继续负责 research/gate/interpretation，OpenCode 负责代码线与远程状态总览。

边界保持：OpenCode 总览不是 route-feasibility review，不授权 execution/training/checkpoint/second smoke/A2/scientific acceptance/qmatch formal acceptance/validated permeability/digital-well claim。
