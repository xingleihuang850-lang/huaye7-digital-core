...
| run_dataset712_*.sh ×3 | 12K | 06-20 | codex | nnU-Netv2 训练脚本 | 不动 |
...
- 仍留根目录（codex 引用，勿动）：`run_dataset712_*.sh`×3、`run_unet3plus_eval_when_done.sh`、`set_env.txt`（nnU-Netv2 env 变量）、`U-Net…QEMSCAN.pdf`。
