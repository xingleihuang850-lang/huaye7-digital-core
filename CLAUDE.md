...
**唯一主线 = 硕士论文《基于融合多尺度数据的页岩数字岩心-井筒建模》**。贡献点 = 生成式(扩散) + 多模态融合 + 数字井筒。**分割只是阶段一地基，不能停在多尺度分割。** 重要修订：**花页7现已有电成像/测井约束数据，因此完整数字井筒闭环先在花页7完成；GJ5-15 后置为外部补充/迁移验证对象，不再是阶段四前置依赖。**

开题四阶段（不可偏离）：**① 分割 → ② 2D 扩散生成 → ③ 多模态 3D 数字岩心 → ④ 数字井筒**。

**当前受控状态（2026-07-12，优先于下列历史 M7/B1 进度叙述）**：阶段一已收尾（E0/E2/E3 配准+3D、S3 Amics 多矿物 1um 丰富相 0.67-0.73；官页15-1-1 T0613论文修改版基准入档见 [`notes/19_阶段一_官页15-1-1_T0613分割基准.md`](notes/19_阶段一_官页15-1-1_T0613分割基准.md)）。阶段二 B1.1 为**有条件阶段性闭合**：冻结主候选是 `ep015`，下游必须显式使用 `hy7-gray-calibration-qmatch-v1`；ORIG raw nnU-Net 是 known fail，不得写成通过。当前授权仅到 calibrated B2-min 的 baseline package、constrained selection smoke、审计和 handoff design；它们**不是 B2 final pass**，也不授权新训练、新 checkpoint、第二次 topology rescue 或放松 gate。阶段三的 smoke/audit/toy 仅为接口或风险诊断，不是 3D 数字岩心、渗透率或数字井筒有效性。当前决策入口：根 [`README.md`](README.md)、[`notes/30_阶段二_B11输出校准与checkpoint选择.md`](notes/30_阶段二_B11输出校准与checkpoint选择.md)、`experiments/花页7_PlanB_记录/phase2/` 的轻量证据。

**历史进度（截至 2026-07-01，用于理解 M7-v2 到 B1 的问题如何出现，不是当前执行授权）**：阶段二 M7-v2 已完成，见 [`notes/22_阶段二_M7v2_阈值标定诊断.md`](notes/22_阶段二_M7v2_阈值标定诊断.md)。
> **M7-v2 结论**：T*=0.98732 标定后分离出两个独立问题——① **阈值伪影（已修）**：S₂ rmse 下降约97%（0.07143→0.00242）；② **连通性真错（待迭代）**：Euler 207.92→真实127.33，孤立散点≠成簇孔隙。
> **M7-v3 / B1 历史状态**：见 [`notes/24_阶段二_M7v3_连通性迭代设计.md`](notes/24_阶段二_M7v3_连通性迭代设计.md)、[`notes/25_阶段二_M7v3_200ep评估与B1决策.md`](notes/25_阶段二_M7v3_200ep评估与B1决策.md)、[`notes/26_阶段二_B1灰度介质生成设计.md`](notes/26_阶段二_B1灰度介质生成设计.md)。`ddpm_ct28_200ep` 已完成采样/评估：200ep T=0 为 φ=4.960%、S₂ rmse=0.00372、Euler=120.88、maxCC=0.0467；T* 标定到 φ=6.4 后 S₂ rmse=0.01010、Euler=119.58。结论：单纯二值 DDPM 加 epoch 不闭合。B1 灰度 `sus` 50ep cheap run 已完成：threshold φ=6.4 口径 S₂ rmse=0.00105、Euler=111.00、maxCC=0.0880，优于二值 DDPM 主线；Fable5 后续控制确认 nnUNet 管线对真实灰度有效，generated gray 的 nnUNet 过分割主要来自灰度域偏移。灰度校正/敏感性实验显示 qmatch 可修正一维灰度分布并把 nnUNet φ 从20.649%降到5.711%、S₂ rmse降到0.00137，但 Euler=108.79、maxCC=0.0824 仍偏；φ=5-8%阈值敏感性中 maxCC稳定偏高约0.082-0.090。后续 B1.1 结论与授权以本节“当前受控状态”为准。
> **阶段三-四主线修订**：旧口径“花页7只做多尺度数字岩心，GJ5-15 才出井筒维度”作废。现在路线是：花页7阶段二 B1/B1.1 生成证据 → 花页7多尺度3D数字岩心 → 花页7电成像/测井约束数字井筒 → GJ5-15 外部补充/迁移验证。
> **接力提示**：新会话先读本节“当前受控状态” + 根 `README.md` + [`notes/30_阶段二_B11输出校准与checkpoint选择.md`](notes/30_阶段二_B11输出校准与checkpoint选择.md) + [`notes/README.md`](notes/README.md) + [`notes/00_研究主线路线图与任务分解.md`](notes/00_研究主线路线图与任务分解.md) + [`notes/03_项目Workflow设计.md`](notes/03_项目Workflow设计.md) + [`notes/04_项目术语表与写法规范.md`](notes/04_项目术语表与写法规范.md)。远程长任务先开 caffeinate（见 memory [[remote-job-no-sleep]]）。
