...
> **M7-v3 / B1 当前状态**：见 [`notes/24_阶段二_M7v3_连通性迭代设计.md`](notes/24_阶段二_M7v3_连通性迭代设计.md)、[`notes/25_阶段二_M7v3_200ep评估与B1决策.md`](notes/25_阶段二_M7v3_200ep评估与B1决策.md)、[`notes/26_阶段二_B1灰度介质生成设计.md`](notes/26_阶段二_B1灰度介质生成设计.md)。`ddpm_ct28_200ep` 已完成采样/评估：200ep T=0 为 φ=4.960%、S₂ rmse=0.00372、Euler=120.88、maxCC=0.0467；T* 标定到 φ=6.4 后 S₂ rmse=0.01010、Euler=119.58。结论：单纯二值 DDPM 加 epoch 不闭合。B1 灰度 `sus` 50ep cheap run 已完成：threshold φ=6.4 口径 S₂ rmse=0.00105、Euler=111.00、maxCC=0.0880，优于二值 DDPM 主线；Fable5 后续控制确认 nnUNet 管线对真实灰度有效，generated gray 的 nnUNet 过分割主要来自灰度域偏移。灰度校正/敏感性实验显示 qmatch 可修正一维灰度分布并把 nnUNet φ 从20.649%降到5.711%、S₂ rmse降到0.00137，但 Euler=108.79、maxCC=0.0824 仍偏；φ=5–8%阈值敏感性中 maxCC稳定偏高约0.082–0.090。下一步应做 B1.1 训练目标/归一化改造或将 B1 200ep 作为受控 scaling check；暂不转 B2。
> **阶段三-四主线修订**：旧口径“花页7只做多尺度数字岩心，GJ5-15 才出井筒维度”作废。现在路线是：花页7阶段二 B1/B1.1 生成证据 → 花页7多尺度3D数字岩心 → 花页7电成像/测井约束数字井筒 → GJ5-15 外部补充/迁移验证。
> **接力提示**：新会话先读本★节 + [`notes/README.md`](notes/README.md) + [`notes/00_研究主线路线图与任务分解.md`](notes/00_研究主线路线图与任务分解.md) + [`notes/03_项目Workflow设计.md`](notes/03_项目Workflow设计.md) + [`notes/04_项目术语表与写法规范.md`](notes/04_项目术语表与写法规范.md) + 22/24/27/28 号笔记。远程长任务先开 caffeinate（见 memory [[remote-job-no-sleep]]）。
