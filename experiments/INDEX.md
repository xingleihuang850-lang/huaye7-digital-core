...
| `experiments/花页7_PlanB_记录/phase2/b1_gray_sus/cheap50/` | B1 灰度 `sus` 50ep cheap run + 512 灰度采样 + threshold/nnUNet 下游评估 | 是 | 待跟踪 | `src/hy7_phase2_ddpm.py`、`src/hy7_phase2_eval.py`、`notes/26_阶段二_B1灰度介质生成设计.md` | 大权重/样本不入 git；本地保存 train_meta、grid、eval metrics/fig、remote logs/hash。threshold 口径：S₂ rmse=0.00105、Euler=111.00、maxCC=0.0880；nnUNet2d 口径过分割 φ=20.649%。 |
